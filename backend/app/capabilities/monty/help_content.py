...
SANDBOX_GUIDANCE = (
    ...
    "or Monty helpers. Use SQL execute with persist_result=true before asking "
    "Monty to transform or visualize query results. Registered helpers are "
    "injected directly into the sandbox namespace, so call helpers such as "
    ...
    "intermediate handle strings, scalar settings, and small derived values. "
    "Pass `restart=True` to `python_sandbox_execute` to reset REPL state when "
    "prior variables or imports are no longer useful. "
    "`get_dataset()` and `preview_dataset()` return preview metadata dictionaries "
    "with columns, row_count, and preview_rows; they do not return a dataframe, "
    "a complete dataset, or a new handle. Do not build charts or transformed "
    "datasets from preview_rows. For real data prep, pass the original dataset "
    "handle to dataframe helpers such as `select_columns()`, `group_by()`, "
    "`melt_columns()`, or `stack_metric_columns()`, then pass the resulting "
    "handle to visualization helpers. "
